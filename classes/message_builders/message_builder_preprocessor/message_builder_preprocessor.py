...
class MessageBuilderPreprocessor:

    @staticmethod
    def build(kafka_object, topic_client_names) -> dict:
        topic_old_name = kafka_object.topic
        topic_new_name = MessageBuilderPreprocessor.attaching_topic_name(topic_old_name, topic_client_names)
        message = kafka_object.value
        clean_text = TextCleaner.clean_text(message['text'])
