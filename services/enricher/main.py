...
from project.classes.message_builders.message_builder_enricher.message_builder_enricher import MessageBuilderEnricher


kafka_host_client = os.getenv("KAFKA_HOST", "localhost")
kafka_port_client = os.getenv("KAFKA_PORT", 9092)

topics_dal = os.getenv("TOPICS_DAL", 'preprocessed_tweets_antisemitic,preprocessed_tweets_not_antisemitic').split(',')
...
topics_client = os.getenv("TOPICS_CLIENT", 'enriched_preprocessed_tweets_antisemitic,enriched_preprocessed_tweets_not_antisemitic').split(',')
...
data_builder = MessageBuilderEnricher
